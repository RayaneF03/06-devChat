...
const Checkout = () => {
  // Estado local da tela de checkout.
  const [carrinho, setcarrinho] = useState(() => {
    const itensCarrinho = localStorage.getItem("devcarrinho");
    return itensCarrinho ? JSON.parse(itensCarrinho) : [];
  });
  ...
  useEffect(() => {
    if (!localStorage.getItem("devcarrinho")) {
      navigate("/");
    }
  }, [navigate]);

  // Finaliza a compra, limpa o carrinho salvo e volta para a home.
  const handleConfirmar = () => {
    alert("Compra Confirmada! Aproveite seu jogo!");
  ...
  // Valida o cupom digitado pelo usuário.
  const aplicarCupom = () => {
    if (cupom.trim().toLowerCase() === "devconstrutor") {
      setCupomAplicado(true);
      setCupomError("");
    } else {
      setCupomAplicado(false);
      setCupomError("Este Cupom Já Foi Utilizado");
    }
  };

  // Limpa o campo do cupom e remove o desconto aplicado.
  const removerCupom = () => {
    setCupom("");
    setCupomAplicado(false);
    setCupomError("");
        ...
        <div className="col-lg-4">
          ...
          >
            ...
            <div className="card-body p-4 pt-3">
              {/* Campo para aplicar ou remover cupom. */}
              <div className="mb-3">
              ...
              {/* Separador visual antes do resumo. */}
              <hr className="my-3" />

              {/* Exibe subtotal e frete. */}
              ...
              {/* Mostra o desconto quando o cupom é válido. */}
              {cupomAplicado && (
              ...
              {/* Segundo separador antes do total. */}
              <hr className="my-3" />
